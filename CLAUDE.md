...
- **Changer d'onglet doit remettre le scroll en haut** (`window.scrollTo(0,0)` + `#main.scrollTop=0` dans le handler de clic de `js/app.js`, juste après `render()`). Bug réel déjà rencontré et corrigé : `render()` remplace `main.innerHTML` mais ne touche jamais à la position de scroll de la fenêtre ; sans ce reset, on atterrit sur le nouvel onglet à la position laissée par le précédent — sur un onglet plus court que celui d'avant, ça donne l'impression d'être bloqué tout en bas de page, incapable de scroller ("je ne peux plus du tout scroll"), alors que le scroll en lui-même n'est pas cassé. Symptôme caractéristique côté utilisateur : la position de scroll d'un onglet "contamine" les onglets suivants. Ne pas déplacer ce reset à l'intérieur de `render()` elle-même (appelée aussi après de simples actions dans le même onglet — ajout d'un repas, coche d'une case — où on ne veut surtout pas sauter en haut de page à chaque fois) : il doit rester spécifique à la navigation réelle entre onglets.
- Le vrai scroll de cette appli est le scroll de **page** (`window`/`document`), pas un scroll interne à `#main` : `#app` a `min-height:100vh` (pas `height`), donc `main{flex:1; overflow-y:auto}` grandit avec son contenu au lieu d'être borné — `overflow-y:auto` sur `main` ne s'active quasiment jamais en pratique dans les tests observés. En diagnostiquer un futur bug de scroll, tester `window.scrollY`/`document.documentElement.scrollHeight`, pas seulement `main.scrollTop`.
- Par prudence, éviter `backdrop-filter` sur `nav.tabs` (ou tout élément `position:fixed` pleine largeur) et toute animation avec `transform` directement sur `#main` : suspectés à tort lors d'un vrai bug de scroll (la cause réelle était le point ci-dessus), mais ce sont des patterns CSS connus pour poser des problèmes de performance/scroll sur certains GPU/pilotes Android — pas de raison de les réintroduire sans un vrai besoin.
