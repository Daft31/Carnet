...
- **Ne jamais mettre `backdrop-filter` sur `nav.tabs` (ou tout élément `position:fixed` pleine largeur)** : testé en prod, a cassé le scroll de toute l'appli sur au moins un téléphone Android réel (probablement un coût de repaint trop élevé à chaque frame de scroll sur certains GPU/pilotes, un problème connu de ce pattern CSS précis). Non reproduit dans les tests Playwright (même avec un vrai swipe tactile simulé via CDP) — un test qui passe en sandbox ne garantit donc rien ici, seul un test sur un vrai téléphone compte pour ce genre de changement.
- **Ne jamais appliquer une animation avec `transform` directement sur `#main`** (le conteneur qui porte `overflow-y:auto`) : risque documenté de casser le scroll tactile sur mobile. Si une transition d'entrée est nécessaire sur le changement d'onglet, se limiter à `opacity` (voir `.tab-enter`/`mainTabIn`), et toujours détacher la classe après coup (`setTimeout`) plutôt que de la laisser accrochée indéfiniment.
- Le vrai scroll de cette appli est le scroll de **page** (`window`/`document`), pas un scroll interne à `#main` : `#app` a `min-height:100vh` (pas `height`), donc `main{flex:1}` grandit avec son contenu au lieu d'être borné — `overflow-y:auto` sur `main` ne s'active quasiment jamais en pratique. En garder conscience avant de diagnostiquer un futur bug de scroll : tester `window.scrollY`/`document.documentElement.scrollHeight`, pas `main.scrollTop`.
