...
// AI-P2-2 : limite de longueur du texte de programme soumis — un programme collé réel
// (même long, plusieurs blocks) tient très largement sous ce seuil ; réduit la surface
// d'abus d'un appel direct avec un payload démesuré.
const MAX_PROGRAM_TEXT_LENGTH = 6000;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (programText.length > MAX_PROGRAM_TEXT_LENGTH) {
      return res.status(400).json({ error: `Programme trop long (max ${MAX_PROGRAM_TEXT_LENGTH} caractères)` });
    }
